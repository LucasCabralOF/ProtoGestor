...
export function Navbar({
  ...
  const groups = useMemo(
    () => buildPrivateNav(t, activeOrg.role),
    [t, activeOrg.role],
  );
          ...
          content={(close) => (
            <div className="min-w-[260px]">
              ...
              <div className="border-t border-(--color-border) p-2">
                <button
                  type="button"
                  data-testid="navbar-button-create-company"
                  className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-dashed border-(--color-border) py-2 text-xs font-semibold text-(--color-text-1) hover:bg-(--color-base-3) transition"
                  onClick={() => {
                    close();
