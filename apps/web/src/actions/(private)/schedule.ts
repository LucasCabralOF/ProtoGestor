...
export const setAppointmentStatusAction = actionClient
  ...
  .action(async ({ parsedInput }) => {
    ...
    let suggestion = null;
    if (parsedInput.status === "done" && existing.recurrenceRule !== "none") {
      suggestion = buildRecurrenceSuggestion({
        baseDate: existing.startsAt,
        recurrenceRule: existing.recurrenceRule,
        serviceOrderId: existing.serviceOrderId,
        serviceTitle: existing.serviceOrder?.title ?? "Visita Técnica",
        customerId: existing.serviceOrder?.customer?.id ?? null,
        customerName: existing.serviceOrder?.customer?.name ?? null,
        customerPhone:
          existing.serviceOrder?.customer?.whatsapp ||
          existing.serviceOrder?.customer?.phone ||
          null,
        locationText: existing.locationText,
        startTime: format(existing.startsAt, "HH:mm"),
        endTime: format(existing.endsAt, "HH:mm"),
      });
    }
...
// ---------------------------------------------------------------------------
// scheduleNextRecurringAppointmentAction — cria 1 próxima visita recorrente
// ---------------------------------------------------------------------------

export const scheduleNextRecurringAppointmentAction = actionClient
  .inputSchema(
    z.object({
      serviceOrderId: z.string().optional().nullable(),
  ...
  .action(async ({ parsedInput }) => {
    ...
    const appointment = await prisma.appointment.create({
      data: {
        ...
        endsAt,
        locationText: parsedInput.locationText?.trim() || null,
        notes: parsedInput.notes?.trim() || "Visita de ciclo recorrente",
        serviceOrderId,
        status: "scheduled",
        recurrenceRule: parsedInput.recurrenceRule,
        recurrenceCount: 1,
